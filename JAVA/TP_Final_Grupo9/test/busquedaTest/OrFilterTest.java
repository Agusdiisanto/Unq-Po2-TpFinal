...
public class OrFilterTest {
	...
	public void setUp() {
		...
		when(proyecto1.includes("Dificultad")).thenReturn(false);
		when(proyecto1.includes("Dificultad")).thenReturn(false);
		...
		compositeOR = new OrFilter(condicion1, condicion2);
		
	}
	
	@Test
	
	public void testFiltrarProyectosOR() {
		
		List<Proyecto> proyectos = Arrays.asList(proyecto1, proyecto2, proyecto3);
		
		List<Proyecto> filtrados = Arrays.asList(proyecto1); 
		
		assertEquals(compositeOR.buscar(proyectos), filtrados);
